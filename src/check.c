...
void binding_check(romen_env* env, ast* top_stmt, int depth) {
	...
	case AST_LET:
		...
		break;
	case AST_CLASS:
		fprintf(stdout, "AST_CLASS(%s):\n", ((ast_class*)top_stmt)->name->buffer);

		env = env_add(env, ((ast_class*)top_stmt)->name->buffer, new_rv_number(1));

		env = env_push(env, (romen_env*)new_env());

		binding_check(env, ((ast_block*)(((ast_class*)top_stmt)->body))->body, depth + 1);
		...
		break;
	case AST_MODULE:
		fprintf(stdout, "AST_MODULE(%s):\n", ((ast_module*)top_stmt)->name->buffer);

		env = env_add(env, ((ast_module*)top_stmt)->name->buffer, new_rv_number(1));

		env = env_push(env, (romen_env*)new_env());

		binding_check(env, ((ast_block*)(((ast_module*)top_stmt)->body))->body, depth + 1);
